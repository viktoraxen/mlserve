...
def test_client_register_model(registered_model: Model):
    assert isinstance(registered_model, Model)


def test_client_register_pytorch_model(client: MLClient):
    model = mlp(3 * 4 * 4, 10, flatten=True)
    model_info = client.register_pytorch_model("pytorch_model", model, input_shape=(3, 4, 4))
...
def test_client_register_onnx_model_with_description(client: MLClient):
    ...
    model = client.register_onnx_model("desc_model", model_path, description="A described model")

    assert isinstance(model, Model)

    models = client.models()
    match = [m for m in models if m.id == model.id]
